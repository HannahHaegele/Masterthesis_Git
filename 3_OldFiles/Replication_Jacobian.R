...
R <- variance_estimate_0*c(1,-1,-1,-1)
...
H <- function(i) {
  a <- c(-(x_prior[2,i]), -(x_prior[1,i]), data1$relativeimportpriceinflation[i], (data1$expect[i] - data1$yoy_inflation[i]))
  
  b <- 1/(x_prior[4,i])* c((x_prior[2,i]), (x_prior[1,i]),  
                           -1/(x_prior[4,i])*(data1$annualized_inflation[i]+(x_prior[2,i])*(x_prior[1,i])-(x_prior[3,i])*data1$relativeimportpriceinflation[i]-data1$yoy_inflation[i]),
                           -data1$relativeimportpriceinflation[i])
  
  c <- 1/(x_prior[4,i])* c((x_prior[2,i]), (x_prior[1,i]),  
                           -1/(x_prior[4,i])*(data1$annualized_inflation[i]+(x_prior[2,i])*(x_prior[1,i])-(x_prior[3,i])*data1$relativeimportpriceinflation[i]-data1$yoy_inflation[i]),
                           -data1$relativeimportpriceinflation[i])
  
  
  d <- 1/(x_prior[3,i])* c((x_prior[2,i]), (x_prior[1,i]), (data1$yoy_inflation[i]-data1$expect[i]),
                           1/(x_prior[3,i])*(-data1$annualized_inflation[i]-(x_prior[2,i])*(x_prior[1,i])-(x_prior[4,i])*(data1$yoy_inflation[i]-data1$expect[i])+data1$yoy_inflation[i]))
...
h2 <- function(i) {
  1/(x_prior[4,i])*(data1$annualized_inflation[i] + (x_prior[2,i])*(x_prior[1,i]) - x_prior[3,i]*data1$relativeimportpriceinflation[i]  - (1-x_prior[4,i])*data1$yoy_inflation[i])
}

h3 <- function(i) {
  1/(1-(x_prior[4,i]))*(data1$annualized_inflation[i] + (x_prior[2,i])*(x_prior[1,i]) - x_prior[3,i]*data1$relativeimportpriceinflation[i]  - x_prior[4,i]*data1$expect[i])
}

h4 <- function(i) {
  1/(x_prior[3,i])*(data1$annualized_inflation[i] + (x_prior[2,i])*(x_prior[1,i]) - x_prior[4,i]*data1$expect[i]  - (1-x_prior[4,i])*data1$yoy_inflation[i])
}
...
for (i in 1:(nrow(data1))) {
  ...
  #update step 
  y[1] <- data1$annualized_inflation[i]-h1(i)
  y[2] <- data1$expect[i]-h2(i)
  y[3] <- data1$yoy_inflation[i]-h3(i)
  y[4] <- data1$relativeimportpriceinflation[i]-h4(i)
  ...
  if (x_post[2,i+1]<0 | x_post[3,i+1]<0 | x_post[4,i+1]<0 | x_post[4,i+1]>1) {
    ...
    if (sum(eigen(Dmat)$values) > 0) Dmat <- nearPD(Dmat)$mat
