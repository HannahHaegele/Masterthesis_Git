...
kalman.log.likelihood.1 <- function(x_prior, x_post, P_prior, P_post, F,Q,R,rho) {
  
  
  h <- function(i) {
    -(x_prior[2,i])*(x_prior[1,i]) + x_prior[3,i]*data1$relative.core.import.price.inflation[i] + x_prior[4,i]*data1$expected.inf[i] + (1-x_prior[4,i])*data1$inflation.yoy[i]
  }
  
  H <- function(i) {
    t(c(-(x_prior[2,i]), -(x_prior[1,i]), data1$relative.core.import.price.inflation[i], (data1$expected.inf[i]-data1$inflation.yoy[i])))
  }
