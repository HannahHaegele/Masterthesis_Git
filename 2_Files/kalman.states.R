...
kalman.states <- function(x_prior, x_post, P_prior, P_post, F,Q,R,rho) {
  ...
  for (i in (nrow(data1)-2):1) {
    ...
    if (x_post_smooth[1,i+1]<0 | x_post_smooth[2,i+1]<0 | x_post_smooth[3,i+1]<0 | x_post_smooth[3,i+1]>1){
