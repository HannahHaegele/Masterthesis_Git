...
format.output <- function(states.smoothed) {
  ...
  colnames(x_post_smooth_data) <- c("kappa","gamma","theta","NAIRU","unemploymentgap","date")
  ...
  for (i in 1:nrow(x_post_smooth_data)) {
    x_post_smooth_data$inflation.predicted[i] <- (x_post_smooth_data$urate - x_post_smooth_data[i,1])*(-x_post_smooth_data[i,2]) + x_post_smooth_data[i,3]*data1$relative.core.import.price.inflation[i] + x_post_smooth_data[i,4]*data1$expected.inf[i] + (1-x_post_smooth_data[i,4])*data1$inflation.yoy[i]
  }
