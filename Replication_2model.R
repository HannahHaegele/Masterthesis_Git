...
for (i in 1:31) {
  ...
  reg <- ConsRegArima(annualized_inflation~ugap+relativeimportpriceinflation+expect_yoy,data=data,order = c(4, 3),
                      constraints ='ugap < 0,relativeimportpriceinflation > 0,expect_yoy > 0',optimizer='mcmc',ini.pars.coef = c(reg0$coeff[1],-0.5,0.1,0.2))

  
  X <- cbind(rep(1,nrow(data)),data$ugap,data$relativeimportpriceinflation,data$expect_yoy) #design matrix of regression model 
